#region Invoke-WalkThrough (Function)
Function Invoke-WalkThrough
{
    ...
    .DESCRIPTION
        Invoke-WalkThrough is an interactive help menu system.  It will convert the static PowerShell help into an interactive menu system
            ...
            o Example of what the Parameter Value should look like in the Help Header
                PARAMETER <parameter>
            ...
            o Example of what the Example Value should look like in the Help Header
                EXAMPLE
                    Command:     Your command string
                    Description: Decription of what the command above will do
                    Notes:       Any Notes

            o Add this to your Function Parameter Section
                [Alias('Help')]
                [Switch]$Walkthrough

            o The below region should be the first Call before running any other code in your script.
	...
	.EXAMPLE
        Command: Invoke-WalkThrough -Help
        Description: Use this to show the help system (using the Invoke-WalkThrough function) as an example.
        Notes:

    .EXAMPLE
        Command: <Verb-Function_Name> -Help to walk through the Help information
        Description: Call your Function with the Alias -Help
        Notes:

    .EXAMPLE
        Command: <Verb-Function_Name> -WalkThrough
        Description: Call your Function with the Parameter -WalkThrough to walk through the Help information
        Notes:

    .EXAMPLE
        Command: Invoke-WalkThrough -Name <Verb-Function_Name>
        Description: This will start the Dynamic help menu system on the called function
        Notes:

    .EXAMPLE
        Command: Invoke-WalkThrough -Name <Verb-Function_Name> -RemoveRun
        Description: This will start the Dynamic help menu system on the called function
	...
	.NOTES
        ...
        • [Original Build Version]
            o 19.02.0505 [XX = Year (.) XX = Month (.) XX = Day XX = Build revision]
        • [Latest Author]
            o Michael Arroyo
	...
	~ Build Version Details "Moved from the Main Help Section.  There is a Char limit and PSHelp will not read all the information correctly":
		o 19.02.0505: [Michael Arroyo] Posted
		o 19.04.0801: [Michael Arroyo] Updated the error control for the entire process to support External Help (XML) files
		...
		o 19.05.2401: [Michael Arroyo] Updated syntax based on PSAnalyzer
		o 19.10.0801: [Michael Arroyo] Rebuilt the entire process to an an external (Only) call.
					: [Michael Arroyo] All Variables are now defined in this script only.  Default values for any parameters are pulled from the
                                        Help Information
					: [Michael Arroyo] Parameter and Example attributes have been updated to provide a better syntax standard and help automate
                                        the menu system.
					: [Michael Arroyo] All functions are -ge PowerShell 2.0 compliant
		o 19.10.3001: [Michael Arroyo] Updated Example Title call to pull the exact match.  Prior, if there was more then 10 examples both 1 and
                                        10 would show when only requesting Example 1.
		o 19.11.2001: [Michael Arroyo] Updated the Parameter Validate Set RegEx Parser to support multiple line scrapping
					: [Michael Arroyo] Updated the Example Set RegEx Parser to support multiple line scrapping
		o 20.11.1301: [Michael Arroyo] Removed the Alias [Invoke-WalkThrough] and copied this script over to a Function Script called
                                        [Invoke-Walkthrough].  Other scripts are looking for a function named [Invoke-Walkthrough], not an Alias.
		o 21.09.0301: [Michael Arroyo] Converted to a Module
	#>
...
#endregion Invoke-WalkThrough (Function)